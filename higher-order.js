...
function popReducer(total, element) {
	return (total += element);
}
let totalPopulation = populations.reduce(popReducer);
...
function strongMonster(element) {
	return element.CP > 200;
}
let myStrongest = monstersInYourPocket.filter(strongMonster);
...
let orderTotals = orders.map(total => total.price * (1 + total.tax));
...
let bobsPrices = bobsPurchases.map(arr => arr.price);
let bobsTotal = bobsPrices.reduce((total, price) => (total += price));
